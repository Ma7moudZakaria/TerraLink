...
public sealed class InstallmentListResponseMapper : IMapHandler<ContractInstallmentEntity, GetInstallmentsOperation.Response>
{
    public GetInstallmentsOperation.Response Handler(ContractInstallmentEntity source)
    {
        decimal paidAmount = source.IncomingPayments.Where(payment => !payment.IsDeleted).Sum(payment => payment.Amount);

        return new GetInstallmentsOperation.Response
        {
            Id = source.Id,
